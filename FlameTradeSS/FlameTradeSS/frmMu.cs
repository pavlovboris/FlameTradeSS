...
namespace FlameTradeSS
{
    public partial class frmMu : Form
    {
        ...
        {
            Mu mu = dgvMu.CurrentRow.DataBoundItem as Mu;
            if (CommonTasks.SendWarningMsg("Сигурни ли сте, че искате да премахнете избраната рестрикция : " + mu.Name + "?") == true)
            {
                if (dgvMu.CurrentRow.Index != -1)
                {
                    if (mu != null)
                    {
                        muBindingSource.Remove(mu);
                        db.Mu.Remove(mu);
                    }
                }
            }
