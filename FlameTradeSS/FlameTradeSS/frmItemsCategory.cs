...
{
    public partial class frmItemsCategory : Form
    {
        public frmItemsCategory()
        {
            InitializeComponent();
        }
        ...
        private void btnAdd_Click(object sender, EventArgs e)
        {
            ...
            itemCategoryBindingSource.Add(newItemCategory);
            itemCategoryBindingSource.MoveLast();
        ...
        {
            ItemCategory itemCategory = dgvItemCategory.CurrentRow.DataBoundItem as ItemCategory;
            if (CommonTasks.SendWarningMsg("Сигурни ли сте, че искате да премахнете избраната рестрикция : " + itemCategory.CategoryName + "?") == true)
            {
                if (dgvItemCategory.CurrentRow.Index != -1)
                {
                    if (itemCategory != null)
                    {
                        itemCategoryBindingSource.Remove(itemCategory);
