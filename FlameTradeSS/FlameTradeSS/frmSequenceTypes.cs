...
{
    public partial class frmSequenceTypes : Form
    {
        public frmSequenceTypes()
        {
            InitializeComponent();
        }
        ...
        {
            SequenceType newSequenceType = new SequenceType();
            sequenceTypeBindingSource.Add(newSequenceType);
            sequenceTypeBindingSource.MoveLast();
            db.SequenceType.Add(newSequenceType);
        }
        ...
        {
            SequenceType sequenceType = dgvSequenceTypes.CurrentRow.DataBoundItem as SequenceType;
            if (CommonTasks.SendWarningMsg("Сигурни ли сте, че искате да премахнете избраната рестрикция : " + sequenceType.TypeName + "?") == true)
            {
                if (dgvSequenceTypes.CurrentRow.Index != -1)
                {
                    if (sequenceType != null)
                    {
                        sequenceTypeBindingSource.Remove(sequenceType);
                        db.SequenceType.Remove(sequenceType);
                    }
                }
            }
        }
